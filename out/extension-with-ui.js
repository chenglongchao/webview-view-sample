...
function activate(context) {
    ...
    vscode.commands.registerCommand("calicoColors.customCommand", (uri) => {
        // 获取当前工作区的根路径
        ...
        if (workspaceFolders) {
            ...
            // 显示信息框
            vscode.window.showInformationMessage(`Custom command executed on: ${relativePath}`);
    ...
    // 注册反向查找引用命令
    vscode.commands.registerCommand("calicoColors.reverseLookup", async (uri) => {
...
// 查找文件引用的函数
async function findFileReferences(targetFilePath, workspaceRoot) {
    const references = [];
...
// 解析导入路径
function resolveImportPath(fromFile, importPath, workspaceRoot) {
...
class ColorsViewProvider {
    ...
    showReferences(fileName, references) {
...
//# sourceMappingURL=extension-with-ui.js.map